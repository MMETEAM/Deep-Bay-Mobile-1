//
//  Activity1ViewController.swift
//  Deep Bay Mobile 1
//
//  Created by Michael Hammond-Todd on 2018-05-22.
//  Copyright © 2018 MMEDesignStudio. All rights reserved.
//  This is the ViewController for the first activity.


import UIKit

class Activity1ViewController: ActivityBaseViewController {
    override var imageFileName: String { return "Activity1.png" }
    override var textKey: String { return "myAnswer1"}
}
